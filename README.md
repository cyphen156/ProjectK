...
게임 아키텍처 설계를 주도하고, 아래 세 시스템을 직접 설계·구현했습니다.

* **[플레이어 시야 시스템](#플레이어-시야-시스템)** : 시야 밖 전투 정보를 렌더링 단계에서 제한
    * 방향성 FOV(마우스 방향 Raycast 방사) + 근접 FOV(원형 메시) 결합으로 실시간 FOV 메시 생성
    * 이진 탐색으로 벽면 경계 구간 보완
    * Stencil 셰이더 3종(Filter·Cover·Mask)으로 오브젝트 필터링
    * 클라이언트 단위 독립 처리 → 플레이어 간 정보 비대칭, 그림자만 남겨 최소 위치 힌트
      
* **[입력 시스템](#입력-시스템)** : 멀티플레이에서 입력이 올바른 대상에게만 적용되도록 제어
    * IPlayerInputReceiver로 입력 대상 추상화, InputManager가 인터페이스 타입으로 전파 대상 관리
    * GameState·InputReceiver enum 기반 입력 범위 제어
    * 플레이어 상태(Die 등) 기반 입력 처리 제한
      
* **[크로스헤어 & 탄착군 시스템](#크로스헤어--탄착군-시스템)** : 조작 상태와 실제 탄착을 시각적으로 일치
    * 이동 상태·조준 여부·부착물 스탯을 종합한 크로스헤어 동적 크기 계산
    * 크로스헤어 크기와 실제 탄 방사 각도 연동
---
